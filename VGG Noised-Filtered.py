...
# Function to load the data from the preprocessed directory
def load_preprocessed_data(data_dir):
    ...
    for fold in folds:
        ...
        if os.path.isdir(foldpath):  # Ensure it is a directory
...
df_preprocessed = load_preprocessed_data(preprocessed_data_dir)


# Change label names to their original names if needed
...
change_label_names(df_preprocessed, 'labels')
...
train_df, ts_df = train_test_split(df_preprocessed, train_size=0.8, shuffle=True, random_state=42)
...
def iou_metric(y_true, y_pred):
    ...
    # Get binary masks from predicted probabilities
