...
def add_gaussian_noise(image, mean=0, var=0.1):
    ...
    noisy = image + gauss
    noisy = np.clip(noisy, 0, 1)  # Clip values to be in the valid range [0, 1]
    return noisy


# Define a function to denoise an image using a median filter
def denoise_image(image):
    image = (image * 255).astype(np.uint8)  # Convert to uint8
    image = cv2.medianBlur(image, 3)
    return image / 255.0  # Convert back to float
...
class CustomImageDataGenerator(ImageDataGenerator):
    ...
    def flow_from_dataframe(self, *args, **kwargs):
        batches = super().flow_from_dataframe(*args, **kwargs)
        self.class_indices = batches.class_indices  # Store class indices

        while True:
            batch_x, batch_y = next(batches)
            noisy_batch = np.array([add_gaussian_noise(x) for x in batch_x])
            denoised_batch = np.array([denoise_image(x) for x in noisy_batch])
            yield denoised_batch, batch_y
...
tr_gen = CustomImageDataGenerator(rescale=1. / 255)
ts_gen = CustomImageDataGenerator(rescale=1. / 255)
