...
# Custom Image Data Generator with Noise and Filtering
# Function to add Gaussian noise
def add_gaussian_noise(image, noise_level=0.05):
    plt.imshow(image)
    plt.title("Before Gau ")
    plt.show()
...
def apply_median_filter(image):
    #filtered_image = median_filter((image * 255).astype(np.uint8), 3)
    ...
    # Debug: Visualize the original image before filtering
    plt.imshow(image)
    ...
    # Debug: Visualize the image after filtering
    plt.imshow(filtered_image)
    plt.title("After Median Filter")
    plt.show()

    # Scale the image back to the range 0-1 for Keras
    return filtered_image.astype(np.float32) / 255.0

# Custom preprocessing function
def custom_preprocessing_function(image, noise_level=0.1):
    plt.imshow(image)
    plt.title("Before Prepro ")
    plt.show()
...
tr_gen = ImageDataGenerator(rescale=1. / 255, preprocessing_function=custom_preprocessing_function)
...
history = cnn_model.fit(train_gen, validation_data=valid_gen, epochs=20, callbacks=[early_stopping, reduce_lr, model_checkpoint])
